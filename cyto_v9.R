# v9: started 24-nov-2023
# TO DO:
# a) calculate max pop'ln size
# b) ANOVA effect of absence/presence
# c) ANOVA effect of richness 

# eventually to do:
# calculate/analyse AUC?
# transform proportions to normal 
...
max_conc %>% ggplot(aes(x = Sample, y = max_conc, color=Sc)) +
  geom_boxplot() +
  labs(y = "max live cells/mL", x = "community") +
  theme_bw()

max_conc %>% ggplot(aes(x = Sc, y = max_conc, color=Sc)) +
  geom_boxplot() +
  labs(y = "max live cells/mL", x = "S. cerevisiae present?") +
  theme_bw()
...
# 
# max_conc %>% 
#   ggplot(aes(x = Sc, y = max_conc, color = Sc)) +
#   geom_boxplot() +
#   labs(y = "max live cells/mL", x = "S. cerevisiae present?") +
#   theme_bw() +
...
# no interacion (species identity)
...
summary(lm_rich) # significant effect of richness
